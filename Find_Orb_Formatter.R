...
for( i in 1:length(asteroids$groupID)){
  ...
  if(month(obs_start) < 10){
    ymd_start = paste0(year(obs_start), " ", paste0("0",month(obs_start)), " ", day(obs_start) + trunc((hour(obs_start)/24 + minute(obs_start)/(24*60) + second(obs_start)/(24*60*60))*10^4)/10^4)
    ymd_end = paste0(year(obs_end), " ", paste0("0",month(obs_start)), " ", day(obs_end) + trunc((hour(obs_end)/24 + minute(obs_end)/(24*60) + second(obs_end)/(24*60*60))*10^4)/10^4)
  }else{
  ...
  line = paste0("     ",asteroids$groupID[[i]],spacer1, "  ", "C",ymd_start, " ", RA_top, " ", Dec_top,"                      X11")
  line2 = paste0("     ",asteroids$groupID[[i]],spacer1, "  ", "C",ymd_end, " ", RA_bottom, " ", Dec_bottom,"                      X11")
  find_orb <- append(find_orb, line)
  find_orb <- append(find_orb, line2)
  cat(nchar(line), nchar(line2))
}
