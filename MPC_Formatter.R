...
formatter <- function(loc, ID, colour, magnitude, RA_vals, Dec_vals){
    ...
    RA_last = paste0(deg2hms(RA_vals[[length(RA_vals)]])[[1]], " ",deg2hms(RA_vals[[length(RA_vals)]])[[2]], " ",deg2hms(RA_vals[[length(RA_vals)]], digits = 2)[[3]])
    Dec_last = paste0(deg2dms(Dec_vals[[length(Dec_vals)]])[[1]], " ",deg2dms(Dec_vals[[length(Dec_vals)]])[[2]], " ",deg2dms(Dec_vals[[length(Dec_vals)]], digits = 2)[[3]])
    ...
    line2 = paste0("     ",ID," KP",ymd_end  , " ", RA_mid  , " ", Dec_mid  ,"         ",mag,colour,"      X11")
    line3 = paste0("     ",ID," KP",ymd_mid  , " ", RA_last , " ", Dec_last ,"         ",mag,colour,"      X11")
    cat(line, "\n")
    cat(line2, "\n")
