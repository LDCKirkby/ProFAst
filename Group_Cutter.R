...
r_image=propaneWarp(r_image_input,keyvalues_out= r_image_header$keyvalues)
i_image=propaneWarp(i_image_input,keyvalues_out= i_image_header$keyvalues)
...
for(i in 1:length(asteroids$groupID)){
  ...
  if(length(which(asteroids$groupID == ID)) > 1){
    colour = rainbow(5)
      ...
      cen_xy = radec2xy(asteroids[asteroids$groupID == ID, "RAcen"], asteroids[asteroids$groupID == ID, "Deccen"], header = image_header)
